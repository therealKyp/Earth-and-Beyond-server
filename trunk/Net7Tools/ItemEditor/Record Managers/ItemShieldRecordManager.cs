...
{
    class ItemShieldRecordManager : IRecordManager
    {
        private IGui m_gui;

        public ItemShieldRecordManager(IGui gui)
        {
            m_gui = gui;
        }
        ...
        {
            switch ((Net7.Table_item_shield)dataBinding.enumeration)
            {
                case Net7.Table_item_shield._rest_race:
                case Net7.Table_item_shield._rest_prof:
                    // Handles both race and profession restrictions
                    return Restrictions.displayField(dataRow, Net7.Table_item_shield._rest_race, Net7.Table_item_shield._rest_prof);
                case Net7.Table_item_shield._cap_100:
                    dataBinding.control.Text = (dataRow == null) ? "45" : ColumnData.GetString(dataRow, dataBinding.enumeration);
                    return true;
                case Net7.Table_item_shield._recharge_100:
                    dataBinding.control.Text = (dataRow == null) ? "1.8" : ColumnData.GetString(dataRow, dataBinding.enumeration);
                    return true;
            }
            return false;
        }
        ...
        public DataValidation.Result validField(Boolean newRecord, DataBinding dataBinding, DataRow dataRow)
        {
            ...
            /*switch ((Net7_db.Table_item_shield)dataBinding.enumeration)
            {
            }*/
        ...
        {
            Net7.Table_item_shield reactorField = (Net7.Table_item_shield)dataBinding.enumeration;
            switch(reactorField)
            {
                case Net7.Table_item_shield._energy_100:
                case Net7.Table_item_shield._range_100:
                    dataRow[ColumnData.GetName(reactorField)] = 0;
                    return true;
                case Net7.Table_item_shield._rest_race:
                case Net7.Table_item_shield._rest_prof:
                    // Handles both race and profession restrictions
                    return Restrictions.updateField(newRecord, dataRow, Net7.Table_item_shield._rest_race, Net7.Table_item_shield._rest_prof);
            }
            return false;
        }
        ...
        public String selectDataRowFromId(String currentId)
        {
            return " SELECT * FROM "
                 ...
                 + " WHERE "
                 + ColumnData.GetName(Net7.Table_item_shield._item_id)
                 + " = "
                 + currentId;
        }
