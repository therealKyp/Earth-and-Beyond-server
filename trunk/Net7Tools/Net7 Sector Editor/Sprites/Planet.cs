...
namespace N7.Sprites
{
    class Planet
    {
        public Planet(PLayer layer, String name, float x, float y, float sigRadius, float rrRadius)
        {
            ...
            if (File.Exists(Path.Combine(dataDirectory, "planet.gif")))
            {
                filePath = "";
            }

            Image image = Image.FromFile(Path.Combine(filePath, Path.Combine(dataDirectory, "planet.gif")));
            ...
            Pen sigPen = new Pen(Color.Blue, 2.0F);
            sigPen.DashStyle = DashStyle.DashDotDot;
            Pen rrPen = new Pen(Color.SkyBlue, 1.0F);
