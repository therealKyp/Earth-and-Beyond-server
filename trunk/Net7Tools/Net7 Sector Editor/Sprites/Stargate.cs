...
namespace N7.Sprites
{
    class Stargate
    {
        public Stargate(PLayer layer, String name, float x, float y, float sigRadius, float rrRadius)
        {
            ...
            if (File.Exists(Path.Combine(dataDirectory, "standardGate.gif")))
            {
                filePath = "";
            }

            Image image = Image.FromFile(Path.Combine(filePath, Path.Combine(dataDirectory, "standardGate.gif")));
            ...
            Pen sigPen = new Pen(Color.ForestGreen, 2.0F);
            sigPen.DashStyle = DashStyle.DashDotDot;
            Pen rrPen = new Pen(Color.GreenYellow, 1.0F);
