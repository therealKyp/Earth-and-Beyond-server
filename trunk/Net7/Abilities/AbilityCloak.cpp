...
void ACloak::EnableDoubleDamage(bool answer)
{
	if(answer)
	{
		...
		if(!m_DoubleDamageActive)
		{
			if(m_BuffID == -1)
			{
				//Yes, I want to modify the base value.
				m_BuffID = m_Player->m_Stats.SetStat(STAT_BUFF_MULT, STAT_BEAM_DAMAGE, 1.0f, "STAT_COMBAT_CLOAK");
			}
			else
			{
	...
	else
	{
		if(m_BuffID == -1)
		{
			m_BuffID = m_Player->m_Stats.SetStat(STAT_BUFF_MULT, STAT_BEAM_DAMAGE, 0.0f, "STAT_COMBAT_CLOAK");
		}
		else
		{
...
bool ACloak::CanUse(int TargetID, long AbilityID, long SkillID)
{
	...
	//TO-DO: Remove me once SQL fix is in place
	if(m_SkillLevel == 0 || m_SkillRank == 0)
	{
		SendError("Advanced Cloak is horribly bugged, consider not using it.");
		SendError("You *might* cloak in 5 seconds");
		m_SkillLevel = 3;
	}
...
*/
void ACloak::Confirmation(bool Confirm, long AbilityID, long GameID)
{
...
bool ACloak::Use(int TargetID)
{
	...
	if(!m_InUse)
	{
		...
		{
			m_Player->SetCurrentSkill(); //default is to set to NULL
			m_DamageTaken = 0;
			return false;
		}
	...
	if(m_InUse)
	{
		...
		CloakEffect.Duration = (short)CloakTime*1000;
...
void ACloak::Update()
{
	if(m_InUse)
	{
		if(m_Player->GetEnergyValue() < CalculateEnergyPerSecond(m_SkillRank))
		{
	...
	else if(m_Player->ShipIndex()->GetIsIncapacitated())
	{
...
bool ACloak::InterruptSkillOnDamage(float damage)
{
	...
	else if(m_DamageTaken >= GetInterruptThreshHold())
	{
		//get new effectID
		m_EffectID = GetNet7TickCount();
...
bool ACloak::InterruptSkillOnMotion(float speed)
{
	...
	else if (m_Player->ObjectIsMoving())
	{
...
{
	if(!m_InUse && !m_Player->ShipIndex()->GetIsCloaked())
	{
		m_Player->SetCurrentSkill();
		return false;
	}
