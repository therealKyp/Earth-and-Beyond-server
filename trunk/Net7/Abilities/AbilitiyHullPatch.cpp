...
*/
void AHullPatch::Confirmation(bool Confirm, long AbilityID, long GameID)
{
	//Not used
}
...
bool AHullPatch::Use(int TargetID)
{
	...
	if(m_InUse)
	{
		//warping is used because it's the only value that will turn off this skill
...
{
	//see if ability was toggled off.
	if(!m_InUse)
	{
	...
	if(m_SkillRank < 3)
	{
		float HullPoints = CalculateHullRepair(m_SkillLevel , m_SkillRank) + m_Player->ShipIndex()->GetHullPoints();
	...
	else if (m_PlayerTarget)
	{
		float HullPoints = CalculateHullRepair(m_SkillLevel , m_SkillRank) + m_PlayerTarget->ShipIndex()->GetHullPoints();
