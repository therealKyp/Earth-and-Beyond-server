...
void Player::SendOpcode(short opcode, unsigned char *data, long length, bool issue)
{
	...
	{
		LogMessage("Can't send packet: %04x [%04x]\n", opcode, length);
	}
}
...
void Player::SendPacketCache()
{
	...
	else
    {
		...
		if (m_SendQueue->CheckQueue(m_ScratchBuffer, &size, MAXIMUM_PACKET_CACHE, m_PacketSequenceNum))
		{
			if (m_SendQueueBk != m_SendQueue)
			{
				LogMessage("!!!!  Send Queue overwritten in SendPacketCache!\n");
				m_SendQueue = m_SendQueueBk;
			}
#if 0
...
#endif
			{
				SendOpcode(ENB_OPCODE_2016_PACKET_SEQUENCE, m_ScratchBuffer, size, true);
				m_PacketSequenceNum++;
			}
		}
...
void Player::IssueTCPBuffer()
{
	if (m_LoginConnection && m_LoginConnection->GameID() == GameID())
	{
		m_LoginConnection->PulseConnectionOutput();
	}
}

//make a separate thread for scanning and re-sending opcodes
void Player::ReSendOpcodes(unsigned char *data)
{
	...
	{
		if (m_SendQueue->RetreiveMessage(m_ScratchBuffer, &size, MAXIMUM_PACKET_CACHE, packet_num, GameID()))
		{
			m_UDPConnection->SendOpcode(ENB_OPCODE_2016_PACKET_SEQUENCE, this, m_ScratchBuffer, size, m_Player_IPAddr, m_Player_Port, packet_num);
		}
		else
		{
		...
		if (m_SendQueueBk != m_SendQueue)
		{
			LogMessage("!!!!  Send Queue overwritten in ReSendOpcodes!\n");
			m_SendQueue = m_SendQueueBk;
		}
	}
}
...
void Player::SendClientSound(char *sound_name, long channel, char queue)
{
...
void Player::HandleSkillStringRequest(unsigned char *data)
{
    ...
    if (obj && obj->ObjectType() == OT_HUSK)
    {
		...
		else if (obj->GetPlayerLootLock() != 0 && GetNet7TickCount() < obj->GetLootTime() && obj->GetPlayerLootLock() != this->GameID()) //loot lock still engaged, and this player is not the credited kill player
		{
			player = g_PlayerMgr->GetPlayer(obj->GetPlayerLootLock());
			if (player == (0)) //player has logged out
			{
				goto loot_as_normal;
			}
			SendVaMessage("%s credited to %s for %.1f seconds", obj->Name(), player->Name(), ((float)obj->GetLootTime() - (float)GetNet7TickCount())*0.001f);
		}
		else
        {
loot_as_normal:
			...
			AwardCredits(obj->GetCreditLoot(), 0, true);
			obj->SetCreditLoot(0);
...
void Player::HandleClientChatRequest(unsigned char *data)
{
	...
	{
		string3 = p_scratch;
		if (length3 > (MAXIMUM_PACKET_CACHE - 32) )
		{
			length3 = (MAXIMUM_PACKET_CACHE - 32); //prevent overrun for exceptionally verbose messages
		}
...
//  sprintf(Nick, "%s", string1);
	...
	else if (request->unknown1 == 0)		// Channel Message
	{
	...
	else if (request->unknown1 == 6)
	{
		//subscribe to channel
		int channel_id = g_PlayerMgr->GetChannelFromName(string2);
		m_ChannelSubscription[channel_id] = true;
	}
	else if (request->unknown1 == 7)
	{
		int channel_id = g_PlayerMgr->GetChannelFromName(string2);
...
void Player::SendAuxNameResource(Object *obj)
{
	...
	//this could have been a stack corruption problem, seems to have gone away now
	...
	switch(InvMo.FromInv)
	{
		...
		case 1:
			...
			else if(InvMo.ToInv == 4)	//selling cargo
			{
				...
				if (Source.Price <= 0)
				{
					SendVaMessage("Vender does not want this item!");
					break;
				}
				...
				long trade_stack = InvMo.Num < Source.TradeStack ? InvMo.Num : Source.TradeStack;
				...
				// calculate profit made on this item
				u64 profit = (Source.Price - (u64)Source.AveCost) * InvMo.Num;
				
                //long XP_earned = 50 * myItem->TechLevel() * InvMo.Num);
				long XP_earned = (long) ((Source.Price > 250 ? 250 : Source.Price - (u64)Source.AveCost) * trade_stack); //profit * 5

				LogMessage("Trade Stack = %d  Profit = %d\n", trade_stack, (Source.Price - (u64)Source.AveCost));

				if (AdminLevel() >= 80 && trade_stack > 0)
				{
					long XP_item = (long) (Source.Price - (u64)Source.AveCost);
					if (XP_item < 0) XP_item = 0;
					//SendVaMessage("NPC ID %d", m_CurrentNPC);
					//SendVaMessage("Trade Stack: %d", trade_stack);
					SendVaMessage("Profit per item: %d", XP_item);
					//SendVaMessage("Trade XP per item: %d", XP_item);
					//SendVaMessage("Quality: %d%", (int)(Source.Quality*100.0f));
				}

				AwardCredits(Source.Price * InvMo.Num, XP_earned);
		...
		case 4:
			...
			if (InvMo.ToInv == 1)	//buy item
			{
				...
				Source.Price = GetVenderBuyPrice(Source.ItemTemplateID);
		...
		case 6:
            {	
                ...
                if (obj)
                {
                    ...
                    {
                    case OT_HUSK:
						if (obj->RangeFrom(Position()) > 2000.0f)
						{
							SendPushMessage("Target too far to engage tractor beam.", "MessageLine", 3000, 3);
						}
						else
						{
...
void Player::HandleRequestTarget(unsigned char *data)
{
	...
	else 
	{
		...
		if (obj && obj->GetUsedInMission() && obj->RangeFrom(Position()) <= 5000.0f)
		{
			...
			CheckForNewMissions(obj->GetDatabaseUID(), 1, obj->GetDatabaseUID());
		}
	}
	SendAuxShip();
    BlankVerbs();
		...
		case 21:	//confirm trade
			{
				...
				if (targetp && targetp->m_TradeConfirm == 1) 
				{
					...
					for(x=0;x<6;x++) 
					{
						...
						targetXP += CalculateTradeStack(targetp->ShipIndex()->Inventory.TradeInv.Item[x].GetData());
					...
					for(x=0;x<6;x++) 
					{
						myXP += CalculateTradeStack(ShipIndex()->Inventory.TradeInv.Item[x].GetData());
					...
					// allocate XP
					if (targetXP > 0) targetp->AwardTradeXP("Trade:", targetXP);
					if (myXP > 0) AwardTradeXP("Trade:", myXP);
...
long Player::CalculateTradeStack(_Item *item)
{
	long XPvalue = 0;
	//does this item have any trade stack value?
	if (item->TradeStack > 0)
	{
		ItemBase * itembase = g_ItemBaseMgr->GetItem(item->ItemTemplateID);
		XPvalue = (itembase->Cost() > 250 ? 250 : itembase->Cost()) * item->TradeStack;
		item->TradeStack = 0;
	}
...
bool Player::MatchOptWithParam (char *option, char *arg, char *&param, bool &msg_sent)
{
...
void Player::HandleSlashCommands(char *Msg)
{
		...
		switch(*pch)
		{
			...
			case 'r':
			{
				...
				else if (strcmp(pch, "rsectors") == 0)
				{
					...
					g_ServerMgr->m_PlayerMgr.ChatSendEveryone(GameID(), "Reloading Sectors and mobs...");
		...
		(Msg[1] != 0) && (!msg_sent || !success))
	{
        ...
        case 'a':
            {
				...
				if (AdminLevel() >= 80 && MatchOptWithParam("addbaseore", pch, param, msg_sent))
				{
		...
		case 'b':
			{
				...
				if (MatchOptWithParam("buff", pch, param, msg_sent))
				{
					...
					TestBuff.EffectID = 214;
					// Set stats for buff
					sprintf(TestBuff.Stats[0].StatName, STAT_SIGNATURE);
					TestBuff.Stats[0].Value = 300;
					TestBuff.Stats[0].StatType = STAT_BUFF_VALUE;

					sprintf(TestBuff.Stats[1].StatName, STAT_SCAN_RANGE);
                    ...
                    {  
                        ...
                        else 
                        {
							...
							if (ItemIDs) 
                            {
								...
								myItem.Structure = 1;
								_snprintf(myItem.BuilderName, 63, "%s GM/DEV", Name());
		...
		case 'n':
			if (strcmp(pch, "noattack") == 0 && AdminLevel() >= 50)
			{
		...
		case 'r':
			...
			else if (AdminLevel() >= 80 && MatchOptWithParam("removebaseore", pch, param, msg_sent))
			{
		...
		case 's' :
			...
			if (MatchOptWithParam("stat", pch, param, msg_sent))
            {
                if (AdminLevel() >= 80)      // Beta to Admin
                {
					char *stat = strtok(param, " ");
...
void Player::WormHole(int sector_id)
{
	...
	if (sm && WeftName)
	{
		...
		Object *weft = om->GetObjectFromName(g_StringMgr->GetStr(WeftName));
		if (weft)
		{
			this->SetPosition(weft->Position());
		}
	}
}
...
bool Player::HandleAggroSetting(char *param)
{
	...
	if (mob && mob_data && (aggro_level >= 0 && aggro_level <=10) && (aggro_level != mob_data->m_Agressiveness))
	{
		...
		sprintf(queryString, "UPDATE `mob_base` SET `aggressiveness` = '%d' WHERE `mob_id` = '%d'", aggro_level, mob->GetMOBType());
		...
		|| obj->ObjectType() == OT_STATION) )
	{
		...
		case 1: //signature
			if (value > 1999)
			{
				...
				//commit to DB
				sprintf(queryString, "UPDATE `sector_nav_points` SET `signature` = '%f' WHERE `sector_object_id` = '%d'", value, s->GetDatabaseUID());
				change = true;
			}
			else
			{
		...
		case 2: //server radius
			...
			sprintf(queryString, "UPDATE `sector_nav_points` SET `object_radius_patch` = '%f' WHERE `sector_object_id` = '%d'", value, s->GetDatabaseUID());
			change = true;
			break;
	...
	if (obj && value != 0 && obj->ObjectType() == OT_FIELD)
	{
		...
		case 1: //radius
			if (value > 1999)
			{
				...
				//commit to DB & re-pop
				sprintf(queryString, "UPDATE `sector_objects_harvestable` SET `max_field_radius` = '%d' WHERE `resource_id` = '%d'", value, f->GetDatabaseUID());
				change = true;
			}
			else
			{
		...
		case 2: //spread type
			...
			//commit to DB & re-pop
			sprintf(queryString, "UPDATE `sector_objects_harvestable` SET `field` = '%d' WHERE `resource_id` = '%d'", value, f->GetDatabaseUID());
			change = true;
			break;
		...
		case 3: //level
			if (value > 0 && value < 9)
			{
				f->SetLevel(value);
				//commit to DB & re-pop
				sprintf(queryString, "UPDATE `sector_objects_harvestable` SET `level` = '%d' WHERE `resource_id` = '%d'", value, f->GetDatabaseUID());
				change = true;
			}
			else
			{
		...
		case 4: //count
			if (value != f->FieldCount())
			{
				...
				//commit to DB & re-pop
				sprintf(queryString, "UPDATE `sector_objects_harvestable` SET `res_count` = '%d' WHERE `resource_id` = '%d'", value, f->GetDatabaseUID());
				change = true;
			}
			break;
		...
		case 6: //add item_id choice to this field
			...
			if (value > 0 && value < 10000)
			{
				...
				sql_result_c result;
				sprintf(queryString, "SELECT * FROM sector_objects_harvestable_oretypes WHERE resource_id = '%d' AND additional_ore_item_id = '%d'", f->GetDatabaseUID(), value);
		...
		case 7: //add base ore to 'base_ore_list'
			//1. check item is valid
			if (value > 0 && value < 10000)
			{				
				...
				sql_result_c result;
				sprintf(queryString, "SELECT * FROM base_ore_list WHERE sector_id = '%d' AND item_id = '%d'", PlayerIndex()->GetSectorNum(), value);
				FieldUpdate.execute(queryString);
				FieldUpdate.store(&result);
		...
		case 8: //remove base ore
			...
			{				
				...
				if (base_ore)
				{
					sprintf(queryString, "DELETE FROM base_ore_list WHERE sector_id = '%d' AND item_id = '%d'", PlayerIndex()->GetSectorNum(), value);
					change = true;
					SendVaMessageC(13, "%s removed from Base Ore list.", base_ore->Name());
				}
			}
			else
			{
...
bool Player::HandleCommitRequest(long Target)
{
	...
	if (obj)
	{
		...
		sprintf(queryString, "UPDATE `sector_objects` SET `position_x` = '%.2f' WHERE `sector_object_id` = '%d'", obj->PosX(), obj->GetDatabaseUID());
	    PositionUpdate.run_query(queryString);
		sprintf(queryString, "UPDATE `sector_objects` SET `position_y` = '%.2f' WHERE `sector_object_id` = '%d'", obj->PosY(), obj->GetDatabaseUID());
	    PositionUpdate.run_query(queryString);
		sprintf(queryString, "UPDATE `sector_objects` SET `position_z` = '%.2f' WHERE `sector_object_id` = '%d'", obj->PosZ(), obj->GetDatabaseUID());
	    PositionUpdate.run_query(queryString);
		sprintf(queryString, "UPDATE `sector_objects` SET `orientation_u` = '%.6f' WHERE `sector_object_id` = '%d'", ori[0], obj->GetDatabaseUID());
	    PositionUpdate.run_query(queryString);
		sprintf(queryString, "UPDATE `sector_objects` SET `orientation_v` = '%.6f' WHERE `sector_object_id` = '%d'", ori[1], obj->GetDatabaseUID());
	    PositionUpdate.run_query(queryString);
		sprintf(queryString, "UPDATE `sector_objects` SET `orientation_w` = '%.6f' WHERE `sector_object_id` = '%d'", ori[2], obj->GetDatabaseUID());
	    PositionUpdate.run_query(queryString);
		sprintf(queryString, "UPDATE `sector_objects` SET `orientation_z` = '%.6f' WHERE `sector_object_id` = '%d'", ori[3], obj->GetDatabaseUID());
	    PositionUpdate.run_query(queryString);
		sprintf(queryString, "UPDATE `sector_objects` SET `scale` = '%.4f' WHERE `sector_object_id` = '%d'", obj->Scale(), obj->GetDatabaseUID());
	    PositionUpdate.run_query(queryString);
		...
		{
		case OT_PLANET:
			sprintf(queryString, "UPDATE `sector_objects_planets` SET `rotate_rate` = '%f' WHERE `planet_id` = '%d'", obj->Spin(), obj->GetDatabaseUID());
			PositionUpdate.run_query(queryString);
			sprintf(queryString, "UPDATE `sector_objects_planets` SET `tilt_angle` = '%f' WHERE `planet_id` = '%d'", obj->Tilt(), obj->GetDatabaseUID());
			PositionUpdate.run_query(queryString);
			break;
		default:
			break;
...
void Player::SendClientChatEvent(int Unknown, int Unknown2, char * FName, char * LName, char * Channel, char * Message)
{
...
void Player::SetStartingPosition()
{
    ...
    else 
    {
        ...
        if (!gate) 
		{
			...
			long sector_id = this->PlayerIndex()->GetSectorNum();
			...
			if (sm && WeftName)
			{
				...
				gate = om->GetObjectFromName(g_StringMgr->GetStr(WeftName));
			}
		}
        if (gate)
        {
...
void Player::HandleIncapacitanceRequest(unsigned char *data)
{
	...
	if (!m_IncapAvatarSent)
	{
		...
		sprintf(avatar.avatar_data.avatar_first_name, "Station\0");
		sprintf(avatar.avatar_data.avatar_last_name, "Mechanic\0");
...
void Player::SendPIPAvatar(long game_id, long avatar_id, bool actual_use)
{
	...
	Object *obj = g_SectorObjects[avatar_id];
	...
	if (actual_use)
	{
		...
		if (len > 20)
		{
			...
			while (centre > 0)
			{
				if (ptr[centre] == ' ')
				{
					strncpy(avatar.avatar_data.avatar_first_name, ptr, centre);
					strncpy(avatar.avatar_data.avatar_last_name, ptr + centre, len-centre);
					split = true;
					break;
				}
				centre--;
		...
		if (!split)
		{
			strncpy(avatar.avatar_data.avatar_first_name, obj->Name(), 20);
