...
typedef map<const char *, MapValue> mapBuffStats;
...
typedef map<const char *, MapType> mapStatValues;

#ifdef WIN32
	typedef void(__stdcall *StatCallBack)(char*);
#else
	typedef void(*StatCallBack)(char*);
#endif
...
typedef map<const char* , StatCallBack> mapStatFuncLookup;
...
public:
	...
	int		SetStat( int BaseType, const char * Stat, float Data, const char * BuffName = "BASE_VALUE");
	bool	DelStat( int BuffID );
	float	GetStat( const char * Stat );
	float	GetStatType(const char *Stat, int Type );
	void	ResetStats();				// Reset all stats
	void	ResetStat( const char * Stat );

private:
	float	CalculateStat(const char * Stat );
	float	FindMaxBuff(const char * Stat,const char *Buff, int Type);
	float	CalculateStatType( const char * Stat, int Type );
	void	CalculateValue(const char * Stat, int Type, const char *Buff);
