...
bool StationLoader::LoadStations()
{
	...
	for(int x=0;x<StationsinDB;x++)
	{
		...
		if (current_station)
		{
            ...
            current_station->Name = g_StringMgr->GetStr((const char*)StationData["name"]);
			...
			const char * welcome = StationData["welcome_message"];
...
void StationLoader::AddRooms(StationTemplate *current_station, long station_id, sql_connection_c *connection)
{
    ...
    sprintf(queryString, "SELECT * FROM `starbase_rooms` WHERE `starbase_id` = '%ld'", station_id);
...
void StationLoader::AddTerminals(StationTemplate *current_station, long room_id, sql_connection_c *connection)
{
    ...
    sprintf(queryString, "SELECT * FROM `starbase_terminals` WHERE `room_id` = '%ld'", room_id);
