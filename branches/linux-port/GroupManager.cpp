...
bool PlayerManager::GroupXP(Player *owner, const char * name, int XP_Gain, int GroupID, int Type)
{
	Group *g = GetGroupFromID(GroupID);
	Player *player = 0;
...
void PlayerManager::AcceptGroupInvite(int GroupID, long PlayerID)
{
    ...
    Player *p = m_GlobMemMgr->GetPlayer(PlayerID);
    p->SetGroup(g);
...
void PlayerManager::RemovePlayerFromGroup(Group * myGroup, int MemberIndex)
{
    ...
    Player *p = m_GlobMemMgr->GetPlayer(myGroup->Member[MemberIndex].GameID);
    p->SetGroup(0);
    p->SetGroupID(-1);
	p->SetAcceptedGroupInvite(false);
...
float PlayerManager::GetGroupWarpSpeed(Player *p)
{
	...
	if (g)
	{
		for(int x=0;x<6;x++)
		{
			//FIXME: Ok, Position is an unsigned int. -1 is obviously signed ... or did you mean 0xFFFFFFFF
...
bool PlayerManager::SetFormation(long leaderID, long formation, const char* formation_name)
{
	Player * leader = GetPlayer(leaderID);
	Player* p = NULL;
	long memberID;
