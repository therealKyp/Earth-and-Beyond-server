...
void Player::SetupAbilities()
{
	...
	*/
	...
	//Repair Equipement ability
	//REGENERATE_EQUIPMENT //rank 1 //effect 178 for user->target ship beam
	//REPAIR_EQUIPMENT //rank 3
	//COMBAT_EQUIPMENT_REPAIR //rank 5
	//AREA_EQUIPMENT_REPAIR //rank 6
	//IMPROVED_AREA_REPAIR //rank 7

	//Shield Charging ability
	//SUPERCHARGE_SHIELDS //rank 1
	//ULTRACHARGE_SHIELDS //rank 3
	//SUPERCHARGE_TARGET //rank 5
	//ULTRACHARGE_TARGET //rank 6
	//MEGACHARGE_SHIELDS //rank 7
...
void Player::SectorReset()
{
	...
	m_QueueCacheIndex = 0;
    memset (m_PacketCache, 0, MAXIMUM_PACKET_CACHE);
...
void Player::FirstLogin()
{
    ...
    if (m_FirstLogin) //this only executes the first time it is run (per player).
    {
		...
		if (ShipIndex()->CurrentStats.GetWarpSpeed() < 0)
		{
			LogMessage("Error: Stats error in negitive\n");
		}
...
void Player::SectorLogin()
{
    ...
    if (PlayerIndex()->GetSectorNum() > 9999) // player is resurrected upon entering starbase - TODO: move to separate method, cost of repair, XP debt
    {
		...
		m_CurrentStation = g_ServerMgr->m_StationMgr.GetStation(PlayerIndex()->GetSectorNum());;
...
void Player::SendSubparts(Player *player_to_send_to)
{
    if (player_to_send_to)
    {
        ...
        subparts.GameID = ntohl(GameID());
        subparts.NumSubParts = ntohl(4);
        subparts.BassetProfession = ntohl(m_Database.ship_info.profession);
        subparts.BassetEngine1 = ntohl(m_Database.ship_info.engine);
        subparts.BassetEngine2 = ntohl(m_Database.ship_info.engine);
        subparts.BassetWing = ntohl(m_Database.ship_info.wing);

        strcpy(subparts.BoneProfession, "~01");
        strcpy(subparts.BoneEngine1, "~02/~03_01");
        strcpy(subparts.BoneEngine2, "~02/~03_02");
        strcpy(subparts.BoneWing, "~02");

        if (m_Database.ship_data.race == RACE_JENQUAI)
        {
            // The Jenquai engines attach to the Profession, not to the Wing
            subparts.BoneEngine1[2] = '1';
            subparts.BoneEngine2[2] = '1';
        }
        player_to_send_to->SendOpcode(ENB_OPCODE_00B4_SUBPARTS, (unsigned char *) &subparts, sizeof(subparts));
...
{
	bool Ignore = false, Answer = false;
    m_Mutex.Lock();
    m_Velocity = 0;
    m_Position_info.SetSpeed = 0.0f;
	...
	m_AbilityList[CLOAK]->SkillInterruptable(&Ignore, &Ignore, &Answer);
	if(ShipIndex()->GetIsCloaked() && Answer)
	{
		m_AbilityList[CLOAK]->InterruptSkillOnAction(WARPING);
	}
    ...
    if (m_WarpDrive && m_WarpNavs[m_WarpNavIndex] != 0)
    {
		...
		if (om)
		{
			...
			if (obj)
			{
				...
				if (m_Velocity > 3500.0f && warp_nav_distance < 7000.0f)
				{
					m_Velocity = 3000.0f;
				}
...
void Player::AwardExploreXP(const char *message, long xp_gain, bool skip_group)
{
    bool GroupXP = false;
    char msg_buffer[128];
	...
	if (!skip_group)
	{
	    ...
	    sprintf(msg_buffer, "%s %ld Explore experience earned", message, xp_gain);
...
void Player::AwardCombatXP(const char *message, long xp_gain, bool group)
{
    bool GroupXP = false;
    char msg_buffer[128];
    ...
    {
        _snprintf(msg_buffer, 128, "%s %ld Combat experience earned", message, xp_gain);
    }
...
void Player::AwardTradeXP(const char *message, long xp, bool skip_group)
{
    char msg_buffer[256];
    ...
    _snprintf(msg_buffer, 256, "%s %ld Trade experience earned", message, xp);
    SendPriorityMessageString(msg_buffer,"MessageLine",5000,4);
...
void Player::SendVaMessage(const char *string, ...)
{
...
void Player::SendVaMessageC(const char colour, const char *string, ...)
{
    unsigned int len = strlen(string) + 256;
    char *pch = (char*)_alloca(len);
