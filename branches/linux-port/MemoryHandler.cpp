    ...
    else
    {
...

        {
            ...
            if (param)
            {
                param->ClassAdd = this;
#ifdef WIN32
                _beginthread(&RunCleanoutThreadAPI, 0, param);
#else
                pthread_create(&m_ThreadHandle, NULL, &RunCleanoutThreadAPI, (void *) param);
#endif
            }
        }
    }
...
void GMemoryHandler::CheckPlayerNodeThread()
{
    ...
    do
    {
		...
		sleep_time = current_time - GetNet7TickCount() + 60000*2; //cleanout every 2 mins
		if (sleep_time < 0) sleep_time = 0;
		Sleep(sleep_time);
    }
    while (!g_ServerShutdown);

    LogMessage("Shutting down check thread");
}
...
Player * GMemoryHandler::GetPlayer(long avatar_id, bool sector_login)
{
    Player *player = 0;
