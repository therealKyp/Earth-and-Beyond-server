...
#define MAX_NAVS_DECOS	512			  //this refers to how many navs and decos we can track the ranges from for the arrival stuff - any decos above that number won't get tracked.
#define TRIGGER_RANGE_1	40000
...
public:
	...
	void	    SendVaMessage(const char *msg, ...);
    void        SendVaMessageC(const char colour, const char *msg, ...);
    void        SendMessageString(const char *msg, char color=5, bool log=true);
...
#else
    static u32 MsgSendThread(void *Param);
#endif
    ...
    void        AwardCombatXP(const char *message, long xp, bool skip_group = false);
    void        AwardExploreXP(const char *message, long xp, bool skip_group = false);
    void        AwardTradeXP(const char *message, long xp, bool skip_group = false);
    ...
    bool        CheckMissions(long object_id, long item_id, short npc_id, completion_node_type completion_type);
    bool        CheckForNewMissions(long object_id, long param_1, short npc_id);
...
private:
    ...
    bool        NPCTalkTree(long mission_slot, long response);
    ...
    void        ActivateProspectBeam(long player_id, long target_id, const char *message, short effect_type, long effectUID, long timestamp, short effect_time);
...
public:
    ...
    void        SendClientSound(const char *sound_name, long channel = 0, char queue = 0);
    ...
    void        SendPushMessage(const char *msg1, const char *type, long time, long priority);
    ...
    void        SendServerHandoff(long from_sector_id, long to_sector_id, const char *from_sector, const char *from_system, const char *to_sector, const char *to_system);
    void        SendNotifyEmote(long game_id, long emote);
    void        SendClientChatEvent(int Unknown, int Unknown2, const char * FName, const char * LName, const char * Channel, const char * Message);
    void        SendRelationship(long ObjectID, long Reaction, bool IsAttacking);
    void        SendGalaxyMap(const char *system, const char *sector, const char *station);
    void        SetStartingPosition();
    void        SendPriorityMessageString(const char *msg1, const char *msg2, long time, long priority);
    void        SendWarpIndex(int index);
    void        SendCreateAttachment(int parent, int child, int slot);

    bool        MatchOptWithParam (const char *option, char *arg, char *&param, bool &msg_sent);

    long        TryLoungeFile(long sector_id);
    void        SendDataFileToClient(const char *filename, long avatar_id=0);
    void        SendDataFileToClientTCP(const char *filename, long avatar_id=0);
...
private:
    ...
    unsigned char     m_PacketCache[MAXIMUM_PACKET_CACHE];
