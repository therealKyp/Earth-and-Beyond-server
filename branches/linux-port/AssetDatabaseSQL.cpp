...
bool AssetContent::LoadAssetContent()
{
	...
	for(unsigned int x=0;x<asset_result->n_rows();x++)
	{
		asset_result->fetch_row(&AssetSQLData);
        current_asset_id = (long)AssetSQLData["base_id"];
        const char *name = AssetSQLData["descr"];
		...
		if (stricmp(name, "NULL") != 0)
		{
			...
			current_asset->m_CatName = g_StringMgr->GetStr(AssetSQLData["main_cat"]);
			current_asset->m_SubCatName = g_StringMgr->GetStr(AssetSQLData["sub_cat"]);
