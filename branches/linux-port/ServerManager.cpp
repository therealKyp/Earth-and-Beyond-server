    ...
    m_IsStandaloneServer(standalone)
{
	...
	m_Connections = new MemorySlot<Connection>(MAX_ONLINE_PLAYERS + 5); //additional 5 so people logging in can be told the server is full. (if it's full).
}
...
void ServerManager::RunMasterServer()
{
	...
	SSL_Listener ssl_listener(m_IpAddressInternal, SSL_PORT, *this);
...
char *ServerManager::GetSystemName(long sector_id)
{
	...
	FILE *OpenLogFile(FILE *logfile, char *name)
	{
	    ...
	    if (!logfile)
	    {
	        ...
	        strftime(timestring, 128, "%Y_%m_%d", ts);
	        sprintf(filename, "%s_%s", name, timestring);
	#endif
	        logfile = fopen(filename, "a+");
	        if(logfile == NULL)
	        {
	        	printf("ERROR: Could not open log file: %s, exiting\n", filename);
	        	exit(1);
	        }
	    }
	    return logfile;
	...
	void LogMySQLMsg(const char *format, ...)
	{
	    char buffer[8192];
	    char timestr[20];
	    ...
	    if (g_ServerMgr)
	    {
			...
			g_ServerMgr->m_ChatFile = OpenLogFile(g_ServerMgr->m_MySQLFile, LogFile);
			fprintf(g_ServerMgr->m_MySQLFile, "%s %s", timestr , buffer);
	    }
	}

	// This function formats a message and adds it to the message queue
	void LogChatMsg(const char *format, ...)
	{
	    char buffer[8192];
	    char timestr[20];
		time_t rawtime;
