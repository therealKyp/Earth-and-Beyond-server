...
void Player::ShipDamage(long source_id, float weapon_damage, short type)
{
    ...
    if (obj && !DebugPlayer())
    {
		...
		******************/
		...
		if(m_AbilityList[CLOAK]->InUse())
		{
			m_AbilityList[CLOAK]->InterruptSkillOnDamage(TakeDamage);
		}
...
float Player::CalcDamage(int weapon_damage, int subcat, bool *critical)
{
    ...
    case 100:		// Beam
		...
		miss_chance -= (short)
			( (m_Stats.GetStatType(STAT_BEAM_ACCURACY, STAT_BUFF_MULT)*1000) +
    ...
    case 101:		// Projectile
		...
		//can't call CalculateStat because STAT_BASE_VALUE = 0, and so any % modifiers would be lost
		miss_chance -= (short)
			( (m_Stats.GetStatType(STAT_PROJECTILES_ACCURACY, STAT_BUFF_MULT)*1000) +
    ...
    case 102:		// Missiles
		...
		//can't call CalculateStat because STAT_BASE_VALUE = 0, and so any % modifiers would be lost
		miss_chance -= (short)
			( (m_Stats.GetStatType(STAT_MISSILE_ACCURACY, STAT_BUFF_MULT)*1000) +
