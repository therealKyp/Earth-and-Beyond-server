...
#else
            pthread_create(&m_PlayerManagerThreadHandle, NULL, &RunMovementThreadAPI, (void *) param);
#endif
        }
    }
}
...
bool PlayerManager::SetupPlayer(Player *player, long IPaddr)
{
	...
	SetIndex(player->GameID(), m_GlobalPlayerList);
...
void PlayerManager::DropPlayerFromGalaxy(Player *p)
{
	...
	p->SavePosition();
    p->SetRemove(); // this ensures the player will be removed the next time the player handling thread processes them
}
...
void PlayerManager::SendPlayerWithoutConnection(long player_id)
{
    ...
    //add this to the global player list
    m_Mutex.Lock();
	SetIndex(player_to_send->GameID(), m_GlobalPlayerList);
...
{
    return (m_GlobMemMgr->GetPlayer(GameID, sector_login));
}
...
void PlayerManager::ListPlayersAndLocations(Player *send_to)
{
	...
	while (GetNextPlayerOnList(p, m_GlobalPlayerList))
	{
        Object *nearest_nav = p->NearestNav();
...
void PlayerManager::GlobalAdminMessage(const char * Message)
{
	Player * p = 0;
    ...
    else
    {
		*/
        ...
        while (GetNextPlayerOnList(p, m_GlobalPlayerList))
		{
            ...
            sprintf(data, "%s		<player name=\"%s\" ip=\"%s\" clevel=\"%ld\" elevel=\"%ld\" tlevel=\"%ld\" race=\"%ld\" profession=\"%ld\"/>\r\n", data, p->Name(),
                inet_ntoa(in),
...
void PlayerManager::SetIndex(u32 index, u32 *player_list)
{
	index &= 0x00FFFFFF;
...
void PlayerManager::UnSetIndex(u32 index, u32 *player_list)
{
	index &= 0x00FFFFFF;
...
bool PlayerManager::GetIndex(u32 index, u32 *player_list)
{
	index &= 0x00FFFFFF;
