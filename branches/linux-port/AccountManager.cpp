...
void AccountManager::SetupTickets()
{
    ...
    void AccountManager::UpdateTime(long account_id)
    {
	    ...
	    sprintf(queryString, "UPDATE `accounts` SET `last_login` = '%s' WHERE `id` = '%ld'", timestr, account_id);
    ...
    bool AccountManager::SetAccountStatus(char *username, long status)
    {
	    ...
	    sprintf(query_str, "UPDATE `accounts` SET `status`='%ld' WHERE `username`='%s'", status, username);
    ...
    void AccountManager::DeleteCharacter(long avatar_id)
    {
        ...
        sprintf(query_str, "DELETE FROM `ship_info` WHERE `avatar_id` = '%ld'", avatar_id);
	    account_query.run_query(query_str);

	    sprintf(query_str, "DELETE FROM `ship_data` WHERE `avatar_id` = '%ld'", avatar_id);
	    account_query.run_query(query_str);

	    sprintf(query_str, "DELETE FROM `avatar_data` WHERE `avatar_id` = '%ld'", avatar_id);
	    account_query.run_query(query_str);

	    sprintf(query_str, "DELETE FROM `avatar_info` WHERE `avatar_id` = '%ld'", avatar_id);
	    account_query.run_query(query_str);

		g_SaveMgr->AddSaveMessage(SAVE_CODE_FULL_CHARACTER_WIPE, avatar_id, 0, 0);

        //Now delete the Aux file
	    sprintf(query_str, "%sCharInfo-%ld.chr", SERVER_USER_PATH, avatar_id);
    ...
    bool AccountManager::ReadDatabase(CharacterDatabase *database, long avatar_id)
    {
        ...
        sprintf(query_str, "SELECT * FROM `avatar_info` WHERE `avatar_id` = '%ld'", avatar_id);
        ...
        database->info.sector_id = ntohl(account_row["sector"]);
        ...
        sprintf(database->info.location, "%s", g_ServerMgr->GetSectorName(account_row["sector"]));

	    ///////////////////////////////////////////////////////////////////////////////////
	    ///////////////////////////    Read in AvatarData   ///////////////////////////////
	    ///////////////////////////////////////////////////////////////////////////////////

        sprintf(query_str, "SELECT * FROM `avatar_data` WHERE `avatar_id` = '%ld'", avatar_id);
        ...
        sprintf(query_str, "SELECT * FROM `ship_data` WHERE `avatar_id` = '%ld'", avatar_id);
        ...
        sprintf(query_str, "SELECT * FROM `ship_info` WHERE `avatar_id` = '%ld'", avatar_id);
...
#else
                    if (!stricmp(restricted_name, create->avatar.avatar_first_name))
#endif
                    {
                        fclose(f);
...
#else
                   	if (!stricmp(restricted_name, create->ship_data.ship_name))
#endif
                    {
                        fclose(f);
...
{
    const char *LOCAL_IP = "127.0.0.1"; //local loopback
    CharacterDatabase database;
