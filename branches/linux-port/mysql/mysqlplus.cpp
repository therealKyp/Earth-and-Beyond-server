...
OPENDB *sql_connection_c::grabdb()
{
    ...
    if (open_db == 0)
    {
        ...
        bool reconnect = true;
        if (mysql_options(&open_db->mysql, MYSQL_OPT_RECONNECT, (const char*)&reconnect))
        {
...
int sql_query_c::execute(char *sql)
{
    ...
    if(mysql_field_count(&odb->mysql) != 0 && res == NULL)
    {
    	return 0;
    }
    return 1;
...
sql_var_c sql_row_c::operator [] (const char * name)
{
	char compound[100];
...
void sql_query::AddData(const char * Field, int Value)
{
	sprintf(m_Values, "%s, '%d'", m_Values, Value);
	AddField(Field);
}

void sql_query::AddData(const char * Field, unsigned long Value)
{
	sprintf(m_Values, "%s, '%ld'", m_Values, Value);
	AddField(Field);
}

void sql_query::AddData(const char * Field, unsigned int Value)
{
	sprintf(m_Values, "%s, '%d'", m_Values, Value);
	AddField(Field);
}

void sql_query::AddData(const char * Field, long Value)
{
	sprintf(m_Values, "%s, '%ld'", m_Values, Value);
	AddField(Field);
}

void sql_query::AddData(const char * Field, float Value)
{
	sprintf(m_Values, "%s, '%f'", m_Values, Value);
	AddField(Field);
}

void sql_query::AddData(const char * Field, double Value)
{
	sprintf(m_Values, "%s, '%f'", m_Values, Value);
	AddField(Field);
}
...
void sql_query::AddData(const char * Field, char Value)
{
	sprintf(m_Values, "%s, '%d'", m_Values, (int)Value);
	AddField(Field);
}

void sql_query::AddData(const char * Field, const char * Value)
{
    char tmp_buff[1024];
...
void sql_query::AddField(const char * Field)
{
...
void sql_query::SetTable(const char * Table)
{
